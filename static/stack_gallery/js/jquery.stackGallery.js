...
(function($) {
	...
	$.stackGallery = function(wrapper, settings) {
	...
	componentPlaylist.css('zIndex',1);//fix for ie9 z index problem (apply to all)
	var indexAddOn=1;
	...
	if(slideshowOn){
		controls_toggle.find('img').attr('src', '/static/stack_gallery/data/icons/pause.png');
	}else{
		controls_toggle.find('img').attr('src', '/static/stack_gallery/data/icons/play.png');
	}
	...
	function setup(){	
		...
		for(i; i <playlistLength; i++){
			...
			div.find('a[class=pp_content]').bind('click touchstart', function(){
				detailOpened=true;
				if(slideshowOn) pauseSlideshow();
				detailActivated();
				return false;
			}).bind('mouseover', function(){
				$(this).find('img').attr('src', '/static/stack_gallery/data/icons/detail_on.png');
				return false;
			}).bind('mouseout', function(){
				$(this).find('img').attr('src', '/static/stack_gallery/data/icons/detail.png');
				return false;
			});
			
			div.find('a[class=pp_link]').bind('mouseover', function(){
				$(this).find('img').attr('src', '/static/stack_gallery/data/icons/link_on.png');
				return false;
			}).bind('mouseout', function(){
				$(this).find('img').attr('src', '/static/stack_gallery/data/icons/link.png');
				return false;
			});
	...
	function getSlideSize(){
		...
		if(useRotation){
			verticalValue = div.outerHeight(false) + 100;//aproximations
			horizontalValue = div.outerWidth(false) + 50;
		}else{
			verticalValue = div.outerHeight(false)+ 10;
			horizontalValue = div.outerWidth(false)+ 5;
		}
	}
	...
	$.stackGallery.checkSlideshow2 = function(){//called after prettyphoto detail close (public function)
		detailOpened=false;
		if(slideshowOn){
			if(!transitionOn){
				resumeSlideshow();
	...
	function overControls(e){
		...
		if(c == "controls_previous"){
			img.attr('src', '/static/stack_gallery/data/icons/prev_on.png');
		}else if(c == "controls_toggle"){
			if(slideshowOn){
				img.attr('src', '/static/stack_gallery/data/icons/pause_on.png');
			}else{
				img.attr('src', '/static/stack_gallery/data/icons/play_on.png');
			}	
		}else if(c == "controls_next"){
			img.attr('src', '/static/stack_gallery/data/icons/next_on.png');
		}
		return false;
	}
	...
	function outControls(e){
		...
		if(c == "controls_previous"){
			img.attr('src', '/static/stack_gallery/data/icons/prev.png');
		}else if(c == "controls_toggle"){
			if(slideshowOn){
				img.attr('src', '/static/stack_gallery/data/icons/pause.png');
			}else{
				img.attr('src', '/static/stack_gallery/data/icons/play.png');
			}	
		}else if(c == "controls_next"){
			img.attr('src', '/static/stack_gallery/data/icons/next.png');
		}
		return false;
	}
	...
	$.stackGallery.toggleSlideshow = function(state) {
	...
	$.stackGallery.toggleDirection = function(dir) {
		if(!componentInited) return;
		if(dir != 'forward' && dir != 'backward') return;
		slideshowDirection = dir;
	}
	
	$.stackGallery.setSlideshowDelay = function(num) {
		if(!componentInited) return;
		slideshowTimeout = num;
	}
	
	$.stackGallery.nextMedia = function() {
		if(!componentInited) return;
		if(transitionOn) return;
		nextMedia();
	}
	
	$.stackGallery.previousMedia = function() {
		if(!componentInited) return;
		if(transitionOn) return;
		previousMedia();	
	}
	
	$.stackGallery.openMedia = function(num) {
		if(!componentInited) return;
		if(transitionOn) return;
		if(num<0)num=0;
	...
	$.stackGallery.openOriginalMedia = function(num) {
		if(!componentInited) return;
		if(transitionOn) return;
		if(num<0)num=0;
		else if(num>playlistLength-1)num=playlistLength-1;
